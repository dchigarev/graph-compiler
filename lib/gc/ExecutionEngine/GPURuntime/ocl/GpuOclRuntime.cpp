...
SmallVector<Type> getArgTypes(const StringRef &funcName, ModuleOp &mod) {
  ArrayRef<Type> args;
  ArrayRef<Type> rets;
  if (auto fn = mod.lookupSymbol<func::FuncOp>(funcName)) {
    args = fn.getArgumentTypes();
    rets = fn.getResultTypes();
  } else if (auto lfn = mod.lookupSymbol<LLVM::LLVMFuncOp>(funcName)) {
    args = lfn.getArgumentTypes();
    rets = lfn.getResultTypes();
  } else {
  ...
  types.reserve(args.size() + rets.size());
  types.append(args.begin(), args.end());
  for (auto t : rets) {
    if (isa<ShapedType>(t)) {
