...
OclModuleBuilder::OclModuleBuilder(ModuleOp module, StringRef funcName)
    : mlirModule(module) {
  if (funcName.empty()) {
    // Find the first public function
    for (auto &op : mlirModule.getBody()->getOperations()) {
    ...
    functionOp = mlirModule.lookupSymbol<func::FuncOp>(funcName);
    if (!functionOp) {
