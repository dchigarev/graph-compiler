...
static bool isFullTransferReadFrom(vector::TransferReadOp readOp, Value tensor) {
...
/// - indices are all constant 0
/// - vector type matches tensor shape and element type
static bool isFullTransferWriteTo(vector::TransferWriteOp writeOp, Value tensor) {
//   llvm::dbgs() << "Checking if transfer_write can be hoisted for tensor iter-arg:\n";
//   tensor.dump();
//   writeOp.dump();
//   if (writeOp.getBase() != tensor)
//     return false;
...
/// If successful, rewrite loop to carry vector instead, and reconstruct tensor after the loop.
static LogicalResult tryHoistTensorAsVector(scf::ForOp forOp,
  ...
  // Ensure the produced tensor from transfer_write is not used elsewhere (besides yield),
  // otherwise we would need to keep it and semantics become more complex.
  ...
  // --- Check that the tensor iter-arg is only read via full transfer_read (and written by that writeOp) ---
  // Allowed uses:
  //  - vector.transfer_read (full) from tensorIterArg
  //  - vector.transfer_write (full) to tensorIterArg (the one that defines yieldedTensor)
  ...
  // Replace init tensor (iter_arg initial value) with transfer_read(initTensor).
  ...
  Value initVec =
      vector::TransferReadOp::create(
        ...
        /*permutationMap*/AffineMap::getMultiDimIdentityMap(tensorTy.getRank(), loc.getContext()), inBounds
    );
  ...
  // New result types: same as old, but tensor result becomes vector result for this slot.
  ...
  IRMapping mapping;
  mapping.map(oldBody->getArgument(0), newBody->getArgument(0)); // induction var
  ...
  for (unsigned i = 0; i < oldRegionIterArgs.size(); ++i) {
    if (i == iterArgIdx)
      mapping.map(oldRegionIterArgs[i], newRegionIterArgs[i]); // tensor -> vector
    else
      mapping.map(oldRegionIterArgs[i], newRegionIterArgs[i]);
  }

  // Clone body ops (excluding terminator), while:
  //  - replacing full transfer_read from old tensor iter-arg with the new vector iter-arg
  //  - skipping the matched transfer_write (we will yield its vector operand instead)
  ...
  for (Operation &op : oldBody->without_terminator()) {
    ...
    // Skip the matched transfer_write that only exists to produce yieldedTensor.
    if (&op == writeOp.getOperation()) {
      // We *don't* need to map its tensor result because we required yieldedTensor.hasOneUse()
      // (only used by scf.yield). If you relax this, you must handle extra uses.
      continue;
    }
  ...
  // Create new yield: for iterArgIdx yield vecToYield (mapped), others yield mapped old operands.
  ...
  // We must preserve original loop result types for users: tensor result is recreated.
  rewriter.setInsertionPointAfter(newFor);

  Value newVecResult = newFor.getResult(iterArgIdx);
  Value reconstructedTensor =
      rewriter.create<vector::TransferWriteOp>(loc, newVecResult, initTensor,
                                               zeroIdx)
          .getResult();
...
struct ForOpHoistTensorToVectorPattern final : OpRewritePattern<scf::ForOp> {
  ...
  LogicalResult matchAndRewrite(scf::ForOp forOp,
    ...
    // Try to apply for any tensor iter-arg; succeed on first successful rewrite.
...
struct HoistForLoopTransferRead final : gc::impl::HoistForLoopTransferReadBase<HoistForLoopTransferRead> {
