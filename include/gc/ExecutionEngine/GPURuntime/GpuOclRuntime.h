...
struct OclModule {
  ...
  bool isStatic() const { return engine->isCtxMain(); }

  bool isOutputArg(unsigned idx) const {
    return (engine->outArgsMask & (1ULL << idx)) != 0;
  }
...
private:
  ...
  template <unsigned N> friend struct StaticExecutor;
  const std::shared_ptr<const gc::cache::CachedEngine> engine;
#ifndef NDEBUG
  const SmallVector<Type> argTypes;
#endif

  explicit OclModule(const OclRuntime &runtime,
                     std::shared_ptr<const gc::cache::CachedEngine> engine,
...
struct OclModuleBuilder {
  ...
  llvm::Expected<OclModule> build(const OclRuntime &runtime);
  llvm::Expected<OclModule> build(cl_command_queue queue);
  llvm::Expected<OclModule> build(cl_device_id device, cl_context context);
...
private:
  ...
  llvm::Expected<OclModule> build(const OclRuntime::Ext &ext);
};
...
protected:
  ...
  explicit OclModuleExecutorBase(const OclModule &mod) : mod(mod) {}
  ...
  void checkExec(const OclContext &ctx) const {
    ...
    assert(argCounter == mod.engine->argCount);
  }
...
template <unsigned N = 8> struct StaticExecutor : OclModuleExecutorBase<N> {
  explicit StaticExecutor(const OclModule &mod)
  ...
  void exec(OclContext &ctx) {
    ...
    std::get<gc::cache::CachedEngine::CtxMainFunc>(this->mod.engine->mainFunc)(
        &ctx, this->args.data());
  }
  ...
  template <typename T> void operator()(OclContext &ctx, T *ptr1, ...) {
    {
      ...
      va_list args;
      va_start(args, ptr1);
      for (unsigned i = 0, n = this->mod.engine->argCount - 1; i < n; i++) {
...
// NOTE: This executor can only be used if !this->mod.isStatic()!
template <unsigned N = 64> struct DynamicExecutor : OclModuleExecutorBase<N> {
  explicit DynamicExecutor(const OclModule &mod)
      : OclModuleExecutorBase<N>(mod) {
    assert(!this->mod.isStatic());
  }
  ...
  void exec(OclContext &ctx) {
    ...
    std::get<gc::cache::CachedEngine::WrappedMainFunc>(
        this->mod.engine->mainFunc)(this->args.data());
